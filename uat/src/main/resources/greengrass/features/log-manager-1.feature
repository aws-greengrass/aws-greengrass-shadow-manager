...
Feature: Greengrass V2 ShadowManager
    ...
    Scenario: ShadowManager-1-T1: configure the shadow manager component using a componentLogsConfiguration list and logs are uploaded to
    CloudWatch
    ...
    Scenario: ShadowManager-1-T1-b: As a customer I can configure the logs uploader component using a componentLogsConfigurationMap and logs are uploaded to CloudWatch
        Given I create a Greengrass deployment with components
            | aws.greengrass.Cli        | LATEST |
            | aws.greengrass.ShadowManager     | LATEST |
        When I update my Greengrass deployment configuration, setting the component aws.greengrass.ShadowManager configuration to:
    ...
    @smoke
    Scenario: ShadowManager-1-T2: As a customer I can configure the logs uploader to delete log files after all logs from the file have been uploaded to CloudWatch
        Given I create a Greengrass deployment with components
            | aws.greengrass.Cli        | LATEST |
            | aws.greengrass.ShadowManager | LATEST |
    ...
    Scenario: ShadowManager-1-T3: As a customer I can configure the logs uploader to delete log oldest log files inorder to keep the disk space limit configured by the customer
    ...
    Scenario: ShadowManager-1-T4: As a developer, logs uploader will handle network interruptions gracefully and upload logs from the last uploaded log after network resumes
        Given I create a Greengrass deployment with components
            | aws.greengrass.ShadowManager | LATEST |
        And I deploy the Greengrass deployment configuration
        Then the Greengrass deployment is COMPLETED on the device after 4 minutes
